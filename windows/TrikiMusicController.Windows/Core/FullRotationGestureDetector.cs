...
public sealed record FullRotationGestureConfiguration(
    ...
    // The two low-pass stages lose roughly 20–25° at the edges of a deliberate turn. A filtered
    // integrated threshold therefore makes the physical gesture finish around the requested target.
    float RequiredRotationDegrees = FullRotationGestureDetector.FilteredRotationTriggerDegrees,
    float MaximumRotationDegrees = 340,
...
{
    public const float PhysicalRotationTargetDegrees = 200;
    public const float FilteredRotationTriggerDegrees = 181;
    ...
    private static void ValidateConfiguration(FullRotationGestureConfiguration value)
    {
        if (value.StabilizationMillis is < 200 or > 3_000 ||
            !float.IsFinite(value.RequiredRotationDegrees) || value.RequiredRotationDegrees is < 80 or > 360 ||
