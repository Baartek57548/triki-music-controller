...
- `FullRotationGestureDetector` po 500 ms stabilizacji wymaga odwróconego kapsla (grawitacja na dodatniej osi Z), bez udziału przycisku całkuje filtrowaną oś żyroskopu Z. Dodatni pełny obrót w prawo wywołuje Next, a ujemny w lewo Previous; próg 330° kompensuje straty filtracji, ale nadal wymaga fizycznego pełnego obrotu. Zatrzymanie, zmiana kierunku, wyjście poza tolerancję akcelerometru lub zbyt długi obrót zerują próbę, po czym detektor automatycznie uzbraja się po uspokojeniu.
...
`TrikiRuntime` jest jedynym miejscem łączącym sensor lub przycisk z akcją. Przekazuje próbki do detektora pełnego obrotu bez warunku przycisku; po rozpoznaniu kierunku wyłącza regulator Z i wysyła Next albo Previous przez `ActionMapper`. Dwa i trzy kliknięcia przycisku pozostają niezależne i są domyślnie mapowane na Like/Dislike. Po każdej interakcji przyciskiem 2-sekundowa stabilizacja kąta rozpoczyna się od nowa. Przerwa strumienia dłuższa niż 250 ms, utrata połączenia, odwrócenie lub przekroczenie 25° także zerują stabilizację i obrót.
