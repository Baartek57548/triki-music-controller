...
- `HoldArcGestureDetector` po 500 ms stabilizacji wymaga odwróconego kapsla (grawitacja na dodatniej osi Z), bez udziału przycisku wyznacza lokalną oś X i estymuje ograniczonym całkowaniem ruch w prawo/lewo oraz pionową głębokość łuku. Kierunek wymaga spójnego czasu, szczytu i impulsu; akcja wymaga 20–30 cm ruchu, obu faz pionowego łuku, hamowania po zauważalnym przemieszczeniu oraz obniżenia prędkości. Ruch prosty, zbyt głęboki, zbyt długi, poza osią lub obrotowy jest unieważniany; prawo daje Like, lewo Dislike, a po akcji detektor samoczynnie uzbraja się po krótkim uspokojeniu ruchu.
...
`TrikiRuntime` jest jedynym miejscem łączącym sensor lub przycisk z akcją. Przekazuje próbki do detektora łuku bez warunku przycisku; po rozpoznaniu ratingu wyłącza regulator Z i odtwarza sygnał zgodny z wynikiem `ActionMapper`. Zwykłe sekwencje przycisku pozostają niezależne. Po każdej interakcji przyciskiem 2-sekundowa stabilizacja kąta rozpoczyna się od nowa. Przerwa strumienia dłuższa niż 250 ms, utrata połączenia, odwrócenie lub przekroczenie 25° także zerują stabilizację i obrót.
...
`TrikiRuntimeEngine` zachowuje priorytet Androida: łuk lewo/prawo na odwróconym kapslu działa bez przycisku, następnie obsługiwane są sekwencje przycisku, a regulator Z działa tylko bez aktywnej interakcji przyciskiem. `MediaControlService` używa Global System Media Transport Controls do Play/Pause/Next/Previous/Stop. `SystemVolumeService` wybiera domyślny endpoint roli `Console` i ustawia jego skalarny poziom Core Audio bez zależności od kroków sterownika; pojedyncza akcja zmienia poziom o 2 punkty procentowe. GSMTC nie ma standardowej akcji oceny utworu, dlatego Like/Dislike wymagałoby adaptera konkretnej usługi; UI i sygnał dźwiękowy jawnie raportują brak wykonania.
